...
- **快速填寫視窗 (Quick Entry)**：
  - 新增 **全域快捷鍵** `Ctrl+Shift+Q` (macOS: `Cmd+Shift+Q`)，可隨時喚出獨立的工時填寫小視窗。
  - **視窗釘選功能 (Pin)**：
    - 支援釘選視窗，防止點擊外部自動隱藏。
    - 釘選後視窗將**保持最上層**。
  - **自動隱藏 (Auto Hide)**：
    - 未釘選狀態下，點擊視窗外部或按下 `Esc` 鍵會自動隱藏視窗。
    - 支援拖曳移動，且拖曳過程中不會誤觸自動隱藏。
  - **禁止調整大小**：固定視窗尺寸，保持介面簡潔。
- **應用程式行為優化**：
  - **關閉邏輯修正**：
    - 若關閉「縮小至系統列」：點擊主視窗關閉按鈕將**完全結束應用程式** (包含快速視窗)。
    - 若開啟「縮小至系統列」：僅隱藏主視窗，保留快速視窗快捷鍵功能。
    - 快速視窗關閉按鈕一律改為隱藏，確保背景服務不中斷。
  - **設定頁面更新**：明確標示關閉選項的影響。
